...
// These use xorshift64 PRNG for reproducibility and don't require proptest.
// ============================================================================
...
fn deterministic_params() -> RiskParams {
    RiskParams {
        warmup_period_slots: 100,
        maintenance_margin_bps: 500,
        initial_margin_bps: 1000,
...
fn fuzz_warmup_budget_invariant() {
    ...
    for seed in 1..=500 {
        ...
        let mut engine = Box::new(RiskEngine::new(deterministic_params()));
            ...
            match action {
                ...
                4 => {
                    ...
                    if account_indices.len() >= 2 {
                        ...
                        let _ = engine.execute_trade(&MATCHER, lp, user, oracle_price, size);
                    }
                }
                5 => {
